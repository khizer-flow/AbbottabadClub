...
void saveUsersToFile(struct UserInfo users[], int totalUsers) {
    ...
    fwrite(users, sizeof(struct UserInfo), totalUsers, file);
    fclose(file);
    printf("Users saved to file successfully.\n");
}
void loadUsersFromFile(struct UserInfo users[], int *totalUsers) {
    FILE *file = fopen("users.txt", "r");
    if (file == NULL) {
        printf("Error opening file for reading. Starting with an empty user list.\n");
        *totalUsers = 0;
        return;
    }
    fread(totalUsers, sizeof(int), 1, file);
    fread(users, sizeof(struct UserInfo), *totalUsers, file);
    fclose(file);
    printf("Users loaded from file successfully.\n");
}


void savePackagesToFile(struct Package packages[], int totalPackages) {
    FILE *file = fopen("packages.txt", "wb");
    if (file == NULL) {
        printf("Error opening file for writing.\n");
        return;
    }
    fwrite(&totalPackages, sizeof(int), 1, file);
    fwrite(packages, sizeof(struct Package), totalPackages, file);
    fclose(file);
    printf("Packages saved to file successfully.\n");
}

void loadPackagesFromFile(struct Package packages[], int *totalPackages) {
    FILE *file = fopen("packages.txt", "r");
    if (file == NULL) {
        printf("Error opening file for reading. Starting with an empty package list.\n");
        *totalPackages = 0;
        return;
    }
    fread(totalPackages, sizeof(int), 1, file);
    fread(packages, sizeof(struct Package), *totalPackages, file);
    fclose(file);
    printf("Packages loaded from file successfully.\n");
}

void saveSubscriptionsToFile(struct Subscription subscriptions[], int totalSubscriptions) {
    FILE *file = fopen("subscriptions.txt", "wb");
    if (file == NULL) {
        printf("Error opening file for writing.\n");
        return;
    }
    fwrite(&totalSubscriptions, sizeof(int), 1, file);
    fwrite(subscriptions, sizeof(struct Subscription), totalSubscriptions, file);
    fclose(file);
    printf("Subscriptions saved to file successfully.\n");
}

void loadSubscriptionsFromFile(struct Subscription subscriptions[], int *totalSubscriptions) {
    FILE *file = fopen("subscriptions.txt", "r");
    if (file == NULL) {
        printf("Error opening file for reading. Starting with an empty subscription list.\n");
        *totalSubscriptions = 0;
        return;
    }
    fread(totalSubscriptions, sizeof(int), 1, file);
    fread(subscriptions, sizeof(struct Subscription), *totalSubscriptions, file);
    fclose(file);
    printf("Subscriptions loaded from file successfully.\n");
}

void saveEmployeesToFile(struct Employee employees[], int totalEmployees) {
    FILE *file = fopen("employees.txt", "wb");
    if (file == NULL) {
        printf("Error opening file for writing.\n");
        return;
    }
    fwrite(&totalEmployees, sizeof(int), 1, file);
    fwrite(employees, sizeof(struct Employee), totalEmployees, file);
    fclose(file);
    printf("Employees saved to file successfully.\n");
}

void loadEmployeesFromFile(struct Employee employees[], int *totalEmployees) {
    FILE *file = fopen("employees.txt", "r");
    if (file == NULL) {
        printf("Error opening file for reading. Starting with an empty employee list.\n");
        *totalEmployees = 0;
        return;
    }
    fread(totalEmployees, sizeof(int), 1, file);
    fread(employees, sizeof(struct Employee), *totalEmployees, file);
    fclose(file);
    printf("Employees loaded from file successfully.\n");
}

void saveCafesToFile(struct Cafe cafes[], int totalCafes) {
    FILE *file = fopen("cafes.txt", "wb");
    if (file == NULL) {
        printf("Error opening file for writing.\n");
        return;
    }
    fwrite(&totalCafes, sizeof(int), 1, file);
    fwrite(cafes, sizeof(struct Cafe), totalCafes, file);
    fclose(file);
    printf("Cafes saved to file successfully.\n");
}

void loadCafesFromFile(struct Cafe cafes[], int *totalCafes) {
    FILE *file = fopen("cafes.txt", "r");
    if (file == NULL) {
        printf("Error opening file for reading. Starting with an empty cafe list.\n");
        *totalCafes = 0;
        return;
    }
    fread(totalCafes, sizeof(int), 1, file);
    fread(cafes, sizeof(struct Cafe), *totalCafes, file);
    fclose(file);
    printf("Cafes loaded from file successfully.\n");
}

void saveFacilitiesToFile(struct Facility facilities[], int totalFacilities) {
    FILE *file = fopen("facilities.txt", "wb");
    if (file == NULL) {
        printf("Error opening file for writing.\n");
        return;
    }
    fwrite(&totalFacilities, sizeof(int), 1, file);
    fwrite(facilities, sizeof(struct Facility), totalFacilities, file);
    fclose(file);
    printf("Facilities saved to file successfully.\n");
}

void loadFacilitiesFromFile(struct Facility facilities[], int *totalFacilities) {
    FILE *file = fopen("facilities.txt", "r");
    if (file == NULL) {
        printf("Error opening file for reading. Starting with an empty facility list.\n");
        *totalFacilities = 0;
        return;
    }
    fread(totalFacilities, sizeof(int), 1, file);
    fread(facilities, sizeof(struct Facility), *totalFacilities, file);
    fclose(file);
    printf("Facilities loaded from file successfully.\n");
}
...
int main() {
    ...
    struct Employee employees[] = {
        {"Khizer Jahangir Khan", "Manager", "0992-111111", "khizer@abbottabadclub.com", "Morning", "Oversees operations"},
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 9:
                ...
                savePackagesToFile(packages, totalPackages);
                break;
            case 10:
                updateSubscription(subscriptions, totalSubscriptions);
                saveSubscriptionsToFile(subscriptions, totalSubscriptions);
                break;
            case 11:
            ...
            case 18:
                printf("\n\n** System Shutdown **\n\n");
                saveUsersToFile(users, totalUsers);
                exit(0);
            default:
